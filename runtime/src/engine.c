#include "haunt/runtime/engine.h"
...
static b8
handle_window_close(Event_Code code, Event_Context* context, void* sender, void* listener) {
...
static b8
handle_window_resize(Event_Code code, Event_Context* context, void* sender, void* listener) {
...
b8 _engine_init(const App_Config* config) {
	...
	if (!platform_start(
			&engine.platform, config->name, config->window.x, config->window.y,
...
b8 _engine_render(void) {
	...
	Color color = color_rgb(
		sin(time) * 2.0 / 2.0, sin(time - 2.0 * pi / 3.0) * 2.0 / 2.0,
